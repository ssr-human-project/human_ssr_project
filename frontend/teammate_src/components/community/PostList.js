...
const PostList = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get('http://localhost:8111/api/posts');
  ...
  return (
    <Container>
      <Sidebar>
        ...
        <MenuButton onClick={() => navigate('/Write-Post')} style={{background: '#ff6b35', color: 'white', marginTop: '20px', textAlign: 'center'}}>
          🖊️ 글쓰기
        </MenuButton>
      </Sidebar>
      ...
      <MainContent>
        ...
        {posts.map((post) => (
          <WideCard key={post.postId} onClick={() => navigate(`/posts/${post.postId}`)}>
            <TextContent>
              <CardHeader>
                <Tag>자유게시판</Tag>
